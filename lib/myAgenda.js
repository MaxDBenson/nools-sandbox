    ...
    instance: {
		...
		**/
        fireNext: function (inBacktrackMode, isFirstFire) {
			...
			if (inBacktrackMode)
			{
				activation = this.getNextActivation(focusedAgenda, isFirstFire);
			} else if (!focusedAgenda.isEmpty()) {
				activation = this.peek();
			}
		...
		getNextActivation: function(focusedAgenda, isFirstFire) {
			var agendaArray,
